...
- **The LinkedIn profile has a spec, and it is the first thing to read before proposing a profile edit** (2026-09-10, after several punch-list items that guessing produced). `job-search/batch-2-spec.md` in the workspace is the browser lane: §3 is About, §4 the experience restructure, §5 skills, §6 Open to Work, each carrying the live text plus the reasoning and the constraints behind it — About is budgeted against a **~250-character above-the-fold block**, verified on the rendered profile, so a sentence added high in it is not a free edit. It is gitignored, like everything under `job-search/`. LinkedIn's own data export (`~/Desktop/Basic_LinkedInDataExport_<date>.zip/`, an unzipped folder despite the name) is the **secondary** source and a point-in-time snapshot: it carries `Profile.csv` and `Positions.csv`, and it goes stale the moment he edits in place, which is exactly how a month-old About got handed back to him as current. **Nothing from either is quoted into this repo** — this one is public, and `Profile.csv` carries his address and birth date in columns beside the text.
