...
- **The LinkedIn consulting entry is shaped differently too: the build section is a paragraph, not bullets** (his call, 2026-09-10). The résumé states the Booth work as two compressed bullets because a résumé bullet is a claim; on LinkedIn that same material is prose, which is what a reader who is already on the profile will actually read. The client lines beneath it (Fleet, DataAnnotation, Artist Growth) stay line-shaped. **So a résumé bullet does not port to LinkedIn as a bullet** — the fact travels and the form is rewritten to the section it lands in. Like About, the live text is not on file here, and the same failure followed from that: a punch list handed over bullet-for-bullet replacements for a section that has no bullets.
