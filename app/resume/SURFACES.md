...
- **The profile's own text has a baseline on disk, and it is the first thing to read before proposing a LinkedIn edit** (found 2026-09-10, after several bad punch-list items that guessing produced). LinkedIn's data export — `~/Desktop/Basic_LinkedInDataExport_<date>.zip/`, an unzipped folder despite the name — carries `Profile.csv` (headline and About) and `Positions.csv` (every position description). It is a **point-in-time snapshot**, not live: he edits the profile by hand between exports, so it answers "what shape is this section, and what does it already say" and never "is this current". **Nothing from it is quoted into this repo** — this one is public, and `Profile.csv` carries his address and birth date in columns beside the text.
