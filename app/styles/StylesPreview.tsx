...
// /styles — type system reference (client half)
//
// Local-only design-system reference. It is a tool, not a portfolio
// artifact: app/styles/page.tsx is the server half and 404s the whole
// route in production, so this component only ever renders under
// `next dev`. The route exists so Malcolm can sanity-check tokens,
// fonts, and sub-brand palettes during the build.
//
// It lives in its own file (rather than inside page.tsx) because the
// production guard has to run on the server before any of this is
// rendered, and a client component cannot carry that guard.
//
// Three sections:
...
// The kicker specimen reuses the real hero status line rather than a
// hand-copied one. The literal that used to sit here had gone stale
// (it still said "LA" and "media & streaming"), which made the
// specimen a worse test of the type than the copy it stands in for.
const KICKER = STATUS;
