...
/** Country ISO-3166 (uppercase) → display name. Unmapped codes pass through. */
...
/** Language ISO-639 (lowercase) → display name. Unmapped codes pass through. */
...
/** Display name for a raw country value (normalizes case, then maps). */
export function countryName(raw: string | null | undefined): string {
  return nameFor(normalizeCountry(raw), COUNTRY_NAMES);
}

/** Display name for a raw language value (normalizes case, then maps). */
export function languageName(raw: string | null | undefined): string {
  return nameFor(normalizeLanguage(raw), LANGUAGE_NAMES);
}
